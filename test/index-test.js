...
describe('index tests', function () {
    ...
    it('read should invoke processTickers for valid file', function () {
        let rawData = 'GOOG\nAAPL\nORCL\nMSFT';
        let parsedData = ['GOOG', 'AAPL', 'ORCL', 'MSFT'];
    ...
    it('getPrice should call get on http with valid URL', function(done) {
        let httpStub = sandbox.stub(stockfetch.http, 'get').callsFake(
            function(url) {
                expect(url)
    ...
    it('getPrice should register handler for failure to reach host',
        function(done) {
            ...
            let onStub = function(event, handler) {
  ...
  2015-09-10,613.099976,624.159973,611.429993,621.349976,1900500,621.349976";
    ...
    it('printReport should not send before all responses arrive', function() {
        ...
        let callbackMock = sandbox.mock(stockfetch)
            .expects('reportCallback')
            .never();
    ...
    it('getPriceForTickers should respond well for a valid file',
        function(done) {
            let onError = sandbox.mock().never();

            let display = function(prices, errors) {
